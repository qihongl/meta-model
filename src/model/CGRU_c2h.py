...
class CGRU_c2h(nn.Module):
    ...
    def __init__(
        ...
        self.c2h = nn.Linear(context_dim, context_dim, bias=bias)
    ...
    def forward(self, x, hidden, context_t=None):
        ...
        context_rep_t = torch.tanh(self.c2h(context_t))
        xc = torch.cat([x, context_rep_t])
        hc = torch.cat([hidden, context_rep_t])
        gate_x = self.i2h(xc)
        gate_h = self.h2h(hc)
        # compute the gates
        ...
        output = [yhat_t, h_t]
        cache = [resetgate, inputgate, newgate, context_rep_t]
        return output, cache
