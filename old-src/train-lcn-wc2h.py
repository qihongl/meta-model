...
from model import CGRU_c2h as Agent
from model import SimpleContext, SimpleMemory, SimpleTracker
from utils import ID2CHAPTER
from utils import EventLabel, TrainValidSplit, Parameters, HumanBondaries, DataLoader
from utils import METAVideos
...
parser.add_argument('--n_pe_std', default=0.1, type=float)
...
def run_model(event_id_list, p, train_mode, save_freq=10):
    ...
    log_llc_ids = [[] for _ in range(len(event_id_list))]
    ...
    log_cr = [[] for _ in range(len(event_id_list))]
    ...
    for i, pi in enumerate(permed_order):
        ...
        print(f'Learning event {i} / {len(event_id_list)} - {log_video_ids[i]}')

        t_start = time.time()
        # get data
        X, log_llc_ids[i] = meta.get_video(log_video_ids[i])
        T = len(X) - 1
        # prealloc
        ...
        log_cr_i = np.zeros((T, dim_context))
        ...
        for t in range(T):
            ...
            log_cr_i[t] = deepcopy(to_np(cache[-1]))
    ...
    result_dict = {
        ...
        'log_h':log_h,
        'log_cr':log_cr,
        'log_video_ids':log_video_ids,
        'log_llc_ids':log_llc_ids,
    }
    result_fname = os.path.join(p.result_dir, f'results-train-{train_mode}.pkl')
